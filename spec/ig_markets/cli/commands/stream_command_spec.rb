describe IGMarkets::CLI::Main, :cli_command do
  def cli
    IGMarkets::CLI::Main.new [], username: '', password: '', api_key: '', accounts: true, trades: true,
                                 markets: ['ABC'], chart_ticks: ['DEF']
  end
  ...
  it 'streams data' do
    ...
    chart_ticks_subscription = instance_double 'Lightstreamer::Subscription'
    ...
    expect(dealing_platform.streaming).to receive(:pop_data).and_return(data: build(:streaming_chart_tick_update))
...
MarketUpdate - bid: 1.11, change: 0.02, change_pct: 1.2, epic: CS.D.EURUSD.CFD.IP, high: 1.2, low: 1.01, market_state: tradeable, mid_open: 1.11, offer: 1.12, strike_price: 1.11, update_time: 2015-12-15 15:00:00 +0000
