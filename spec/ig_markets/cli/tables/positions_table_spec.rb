describe IGMarkets::CLI::PositionsTable do
  it 'prints positions' do
    positions = [build(:position), build(:position, level: 100.1, contract_size: 100)]

    expect(IGMarkets::CLI::PositionsTable.new(positions).to_s).to eql(<<-END.strip
+-------------------------+--------------------+------------+-----------+------+-------+---------+-------+------+-------+------+-------------+----------+
|                                                                       Positions                                                                       |
+-------------------------+--------------------+------------+-----------+------+-------+---------+-------+------+-------+------+-------------+----------+
...
END
  ...
  it 'prints positions in aggregate' do
    ...
    expect(IGMarkets::CLI::PositionsTable.new(positions, aggregate: true).to_s).to eql(<<-END.strip
+------+--------------------+------------+-----------+------+-------+---------+-------+------+-------+------+--------------+------------+
|                                                               Positions                                                               |
+------+--------------------+------------+-----------+------+-------+---------+-------+------+-------+------+--------------+------------+
| Date | EPIC               | Type       | Direction | Size | Level | Current | High  | Low  | Limit | Stop | Profit/loss  | Deal IDs   |
